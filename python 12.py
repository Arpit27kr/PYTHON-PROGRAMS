...
mycursor=mydb.cursor(buffered=True) #Created Database BankDB #mycursor.execute('create database BankDB')
...
def Insert():
    while True: #Loop for accepting records Acc=input("Enter account no") Name=input("Enter Name")
...
def DispSortName():   #Function to Display records as per
...
def DispSortBal(): #Function to Display records as per
        ...
        for i in S:
            ...
            if i[0]==A:
                ...
                if ch=='y' or ch=='Y':
                    ...
                    cmd="UPDATE BANK SET NAME=%s,MOBILE=%s,EMAIL=%s,ADDRESS=%s,CITY=%s,COUNTRY=%s,BALANCE=%sWHERE ACCNO=%s"
